...
public final class CastleWars {
    ...
    public static void joinPortal(Player player, int team) {
        ...
        if (playingGame == null && waiting[team].size() >= 1) // players needed on each team to start game
            createPlayingGame();
        else
        ...
        .sendConfig(
                ...
                playingGame == null ? 0
                        ...
                        - (player.getControlerManager()
                                .getControler() instanceof CastleWarsPlaying ? 1
                                        : 0));
    }
    ...
    public static void removePlayingPlayer(Player player, int team) {
        ...
        player.getInventory().deleteItem(590, Integer.MAX_VALUE); // tinderbox
        player.getInventory().deleteItem(954, Integer.MAX_VALUE); // rope
        player.getInventory().deleteItem(1265, Integer.MAX_VALUE); // bronze pickaxe
        player.getInventory().deleteItem(1925, Integer.MAX_VALUE); // bucket
        player.getInventory().deleteItem(1929, Integer.MAX_VALUE); // bucket of water
        player.getInventory().deleteItem(4043, Integer.MAX_VALUE); // rocks
        player.getInventory().deleteItem(4045, Integer.MAX_VALUE); // rocks
        player.getInventory().deleteItem(4049, Integer.MAX_VALUE); // bandages
        player.getInventory().deleteItem(4051, Integer.MAX_VALUE); // toolkit
    ...
    private static class PlayingGame extends TimerTask {
        ...
        public void addBarricade(int team, Player player) {
            if (barricadesCount[team] >= 10) {
                player.getPackets()
                .sendGameMessage(
                        "Your team has already set up ten barricades.");
                return;
            }              
        ...
        public void takeFlag(Player player, int team, int flagTeam,
            ...
            if (flagTeam != team
                    && (player.getEquipment().getWeaponId() == 2 || player
                    .getEquipment().getShieldId() == 2)) {
