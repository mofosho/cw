			...
			if (player.getRights() == 10 && playerIsAdmin(player)) {
				players.getPackets().sendGameMessage(
						"<col=ff0000>[Main Owner] <img=10>"
								...
								+ message + "</col>");
	    } else if (player.getRights() == 10 && player.getUsername().equalsIgnoreCase("destiny")) {
                players.getPackets().sendGameMessage(
                        "[<col=736AFF>Cardgames on Motorcycles</col>] <img=1>"
                        + player.getDisplayName() + ": <col=736AFF>"
                        + message + "</col>");
            } else if (player.getRights() == 10 && player.getUsername().equalsIgnoreCase("zingerman30")) {
                players.getPackets().sendGameMessage(
                        "[<col=FF0000>Owner</col>] <img=10>"
                        + player.getDisplayName() + ": <col=ff0000>"
                        + message + "</col>");
            } else if (player.getRights() == 10 && player.getUsername().equalsIgnoreCase("abyssal")) {
                players.getPackets().sendGameMessage(
                        "[<col=000000>Leader</col>] <img=10>"
                        + player.getDisplayName() + ": <col=000000>"
                        + message + "</col>");
			} else if (player.getRights() == 10 && player.getUsername().equalsIgnoreCase("Money_talks")) {
                players.getPackets().sendGameMessage(
                        "[<col=00FF00>Beast Owner</col>] <img=10>"
                        + player.getDisplayName() + ": <col=00FF00>"
                        + message + "");
			} else if (player.getRights() == 2 && player.getUsername().equalsIgnoreCase("krissie")) {
                players.getPackets().sendGameMessage(
                        "[<col=FF00FF>Princess</col>] <img=1>"
                        + player.getDisplayName() + ": <col=FF00FF>"
                        + message + "</col>");
			} else if (player.getRights() == 10 && player.getUsername().equalsIgnoreCase("god")) {
                players.getPackets().sendGameMessage(
                        "[<col=FF0000>Forums Developer</col>] <img=10>"
                        + player.getDisplayName() + ": <col=FF0000>"
                        + message + "</col>");
			} else if (player.getRights() == 1 && player.getUsername().equalsIgnoreCase("denoo")) {
                players.getPackets().sendGameMessage(
                        "[<col=33CCFF>Head Mod</col>] <img=0>"
                        + player.getDisplayName() + ": <col=33CCFF>"
                        + message + "</col>");
			} else if (player.getRights() == 2) {
				players.getPackets().sendGameMessage(
						"[<col=FFFF33>Admin</col>] <img=1>"
				...
				players.getPackets().sendGameMessage(
						"[<col=00BFFF>Supporter</col>]<img=12>"
								+ player.getDisplayName() + ": <col=00BFFF>"
								+ message + "</col>");
			} else if (player.isDonator()) {
				players.getPackets().sendGameMessage(
						"[<col=00ff00>Donator</col>] <img=8>"
								+ player.getDisplayName() + ": <col=00ff00>"
								+ message + "</col>");
			} else if (player.isExtremeDonator()) {
				players.getPackets().sendGameMessage(
						"[<col=20f2eb>Extreme Donator</col>] <img=11>"
				...
				players.getPackets().sendGameMessage(
						"[<col=FF6600>Co-Owner</col>]<img=10>"
								+ player.getDisplayName() + ": <col=FF6600>"
								+ message + "</col>");	
			} else if (player.isSupporter()){
				players.getPackets().sendGameMessage(
						"[<col=347235>Supporter</col>]<img=12>"
								+ player.getDisplayName() + ": <col=347235>"
								+ message + "</col>");
			} else if (player.isDonator()) {
				players.getPackets().sendGameMessage(
	...
	public static boolean processAdminCommand(final Player player,
		...
		if (clientCommand) {
			...
			case "sgar":
				...
				//CastleWars
			case "cw":
				player.getControlerManager().startControler("CastleWarsWaiting");
				return true;
			case "scg":
			...
			case "karamja":
				...
				/*
				 * case "shop": ShopsHandler.openShop(player,
				 * Integer.parseInt(cmd[1])); return true;
				 */
	...
	public static boolean processModCommand(Player player, String[] cmd,
		...
		if (clientCommand) {
			...
			/*case "edge":
				...
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3087,
						3505, 0));
				return true;*/
			case "aura":
				if (playerIsAdmin(player)) {
	...
	public static boolean processNormalCommand(Player player, String[] cmd,
		...
		if (clientCommand) {
			...
			case "players":
				...
				return true;
			case "home":
				/*Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3225,
						3218, 0));*/
				player.getPackets().sendGameMessage("This was removed because of bug abuse abuse. Use the Lodestone instead.");
				return true;

			case "ge":
				//Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3167,
				//		3460, 0));
				player.getPackets().sendGameMessage("This was removed because of bug abuse abuse. Use the Lodestone instead.");
				return true;
			...
			case "giveadminmeme":
				 ...
				 * "Enter the display name you wish:"); return true;
				 */
			case "donatorcity":
				if (!player.isDonator() && !player.isExtremeDonator()) {
					player.getPackets().sendGameMessage(
			...
			case "vote":
				player.getPackets().sendOpenURL(
						"http://NRS3server.info/forums.html");
				return true;
			...
			case "donate":
				...
				return true;
			case "tinychat":
				player.getPackets().sendOpenURL("http://NRS3server.info");
				return true;
			case "itemdb":
			...
			case "copysd":
				...
				for (int i = 0; i < items.length; i++) {
					...
					for (String string : Settings.EXTREME_DONATOR_ITEMS) {
						if (!player.isExtremeDonator()
								&& items[i].getDefinitions().getName()
