...
import com.rs.content.skills.summoning.SummoningPouch;
...
public class Summoning implements Serializable {
	...
	public static final class Pouches implements Serializable {
		...
		static {
			SPIRIT_WOLF = new Pouches("SPIRIT_WOLF", 0, 6829, 67, 12047, 1,
	...
	public static void sendInterface(Player player) {
		...
		Object options[] = { Integer.valueOf(78), Integer.valueOf(1),
				"List<col=FF9040>", "Infuse-X<col=FF9040>",
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		...
		if (skillReq != null) {
			for (Iterator<?> iterator = skillReq.keySet().iterator(); iterator
					.hasNext();) {
				int skillId = ((Integer) iterator.next()).intValue();
				if (skillId <= 24 && skillId >= 0) {
					int level = ((Integer) skillReq.get(Integer
							.valueOf(skillId))).intValue();
					if (level >= 0
							&& level <= 120
						...
						player.getPackets()
								.sendGameMessage(
										(new StringBuilder("You need to have a"))
												.append(name.startsWith("a") ? "n"
														: "").append(" ")
												.append(name)
												.append(" level of ")
												.append(level)
												.append(" to summon this.")
												.toString());
					}
				}
			}
	...
	public static void createPouch(final Player player, int itemId, int amount) {
		SummoningPouch pouch = SummoningPouch.get(itemId);
		if (pouch == null) {
			player.sendMessage("You do not have the items required to create this pouch.");
			return;
		}
		...
		if (i == 1) {
			player.sendMessage("You infuse a "
					+ ItemDefinitions.getItemDefinitions(itemId).getName()
							.toLowerCase() + ".");
		} else if (i > 0) {
			player.sendMessage("You infuse some "
					+ ItemDefinitions.getItemDefinitions(itemId).getName()
							.toLowerCase() + "es.");
		} else {
			return;
