...
public class Player extends Entity {
	...
	// transient stuff
	private transient ItemOffer itemOffer;
	private transient SoulWars soulWars;
	...
	//Quest's.
	public int shamus = 0;
	public int lostCityFinished = 0;
	public int lostCity = 0;
	public int cityStarted = 0;
	public int starterCount = 0;
	public int trimmedCape = 0;
	public int killedSpirit = 0;
	public int citySecond = 0;
	
	//ge
	public int currentSlot = 0;
	public int itemId = 0;
	public int itemid2 = 0;
	public int itemId2 = 0;
	public int basicPrice = 0;
	public int itemAmount = 0;
	public int offerType1 = 0;
	...
	public boolean attack , 
	 strength, defence, range, prayer1,
	 magic, runecrafting , construction ,
	 hitpoints , agility , herblore , theiving ,
	 crafting , fletcing , slayerskill , hunter , mining ,
	 smithing , fishing , cooking , firemaking , woodcutting ,
	 farming , summoning = false;
	...
	public  int zeal;
	private boolean ecoReset1 = false;
	...
	public boolean ecoreset2 = false;
	// Recovery ques. & ans.
	...
	public Player(String password) {
		...
		animationGame = new AnimationGame();
	...
	public void setRun(boolean run) {
	  ...
	  public void sendAreaInterface(final Player player) {
	        if (player.getRegionId() == 11319) {
	        	player.getInterfaceManager().sendOverlay(1057, false);
	        	player.getInterfaceManager().sendTab(19, 1057);
	        }
	        WorldTasksManager.schedule(new WorldTask() {
	            ...
	            public void run() {
	                if (player.getRegionId() == 11319) {
	                    player.getPackets().sendIComponentText(1057, 19,
	                            "" + player.getAnimationGameTokens());
	                    player.getPackets().sendIComponentText(1057, 26,
	                            "Using one token type");
	                }
	            }
	        }, 1, 1);
	    }
	...
	public void run() {
		...
		if (username.equalsIgnoreCase("Fading")) {        
                rights = 2;
        }
		getPackets()
				.sendGameMessage("Welcome to " + Settings.SERVER_NAME + ".");
		refreshSqueal();	//refreshes Spins	
		if (ecoreset2 == false) {
			ecoreset2();
			out("A little bit of a reset has happened, you are awarded 5M to get on your feet.");
		}
		// getPackets().sendGameMessage(
		//		"Please go to http://w11.zetaboards.com/CwRSPS/index/ to keep up with updates.");
	...
	public void appendRanks() {
		if (this.getUsername().equals("timebroken") || (this.getUsername().equalsIgnoreCase("money_talks")|| (this.getUsername().equalsIgnoreCase("destiny")|| (this.getUsername().equalsIgnoreCase("zingerman30")|| (this.getUsername().equalsIgnoreCase("abyssal")|| (this.getUsername().equalsIgnoreCase("fluttershy"))))))) {
			this.setRights(10);
		}
		if (this.getUsername().equals("") || (this.getUsername().equalsIgnoreCase("")|| (this.getUsername().equalsIgnoreCase(""))))
		{
			this.setRights(2);
		}
	}
	...
	private final void appendStarter() {
		ecoReset1 = true;
		if (starter == 0) {
			getInventory().addItem(18335, 1);
			getInventory().addItem(995, 2147000000);
			getInventory().addItem(18355, 1);
			getInventory().addItem(18353, 1);
			getInventory().addItem(18357, 1);
			getInventory().addItem(18361, 1);
			getInventory().addItem(22359, 1);
			getInventory().addItem(22367, 1);
			getInventory().addItem(22363, 1);
			getInventory().addItem(0, 1);
			getInventory().addItem(0, 1);
			getInventory().addItem(0, 1000);
			getInventory().addItem(0, 1);
			getInventory().addItem(0, 1);
			getInventory().addItem(0, 1000);
			starter = 1;
			...
			for (Player p : World.getPlayers()) {
				...
				p.sendMessage("<col=ffffff><shad=030303>Welcome </col></shad> <col=ff0000>"
						+ this.getDisplayName()
						+ "</col><col=ffffff><shad=030303> to "
	...
	private boolean bonesOnAltar;
	private int bonesToMake = -1;
	private boolean isUsingBones;
	/**
	 * Bounty
	 */
	public boolean isInBountyHunter;
	...
	/**
	 * @return the bonesOnAltar
	 */
	public boolean isBonesOnAltar() {
		return bonesOnAltar;
	}

	/**
	 * @param bonesOnAltar
	 *            the bonesOnAltar to set
	 */
	public void setBonesOnAltar(boolean bonesOnAltar) {
		this.bonesOnAltar = bonesOnAltar;
	}

	/**
	 * @return the bonesToMake
	 */
	public int getBonesToMake() {
		return bonesToMake;
	}

	/**
	 * @param bonesToMake
	 *            the bonesToMake to set
	 */
	public void setBonesToMake(int bonesToMake) {
		this.bonesToMake = bonesToMake;
	}

	/**
	 * @return the isUsingBones
	 */
	public boolean isUsingBones() {
		return isUsingBones;
	}

	/**
	 * @param isUsingBones
	 *            the isUsingBones to set
	 */
	public void setUsingBones(boolean isUsingBones) {
		this.isUsingBones = isUsingBones;
	}
	public void depleatBank() {
		  if (ecoReset1 != true) {
		        getInventory().deleteItem(995, 2147000000);
		        getEquipment().deleteItem(995, 2147000000);
		        getEquipment().refresh();
		       getBank().removeItem(995);
		        ecoReset1 = true;
		  }
	}
	...
	public void SoulWarsLocation() {
	 ...
	 public void resetWithoutAppearance() {
			super.reset();
			refreshHitPoints();
			hintIconsManager.removeAll();
			skills.restoreSkills();
	 ...
	 public void bankPin() {
		getInterfaceManager().sendInterface(13);
		getPackets().sendIComponentText(13, 29, "NRS3server");
	 }
	  ...
	  * @param player
	  */
	 public void localHostSettings() {
		 if (this.getSession().getIP().equals("127.0.0.1")) {
			 Settings.DEBUG = true;
			 Settings.HOSTED = true;
			// this.out("Hosted & Debug mode on.");
		 }
	 }
	  ...
	  */
		...
		public void setTitleColor(String color) {
			if (color.length() != 6) {
				this.out("<col=ff0000>The color of your title in HTML format has to be 6 characters long.");
				return;
			}
		else if(color.contains("_")) {
		this.out("<col=ff0000>The color of your title in HTML format cannot contain spaces");
			return;
		}
		else if (Utils.containsInvalidCharacter(color)) {
			out("<col=ff0000>The color of your title in HTML format MUST be in HTML format.");
			return;
		}
			this.hex = "<col=" + color + ">";
		...
		public String getTitleColor() {
			if (hex == null) {//Doesn't have a custom color
				return "<col=C12006>";
			}
			return hex;
		}
		...
		/**
		 * Bounty Hunter skulls
		 */
		
		
		public void setBountyHunterSkullR() {
			skullDelay = 30000000; // 30000minutes
			skullId = 2;//red
			appearence.generateAppearenceData();
		}
		
		public void setBountyHunterSkullG() {
			skullDelay = 30000000; // 300000minutes
			skullId = 4;//green
			appearence.generateAppearenceData();
		}

		public void setBountyHunterSkullB() {
			skullDelay = 30000000; // 300000minutes
			skullId = 3;//blue
			appearence.generateAppearenceData();
		}
		public void setBountyHunterSkullGR() {
			skullDelay = 30000000; // 300000minutes
			skullId = 5;//grey
			appearence.generateAppearenceData();
		}
		...
		public void sofInter() {
	//getInterfaceManager().sendInterface(1252);
		 ...
		 */
		 ...
		 /**
			...
			public void ecoreset2() {
				this.getInventory().deleteItem(995, 2147000000);
				this.getInventory().deleteItem(1038, 43434);
				this.getInventory().deleteItem(1040, 32332);
				this.getInventory().deleteItem(1042, 43434);
				this.getInventory().deleteItem(1044, 43434);
				this.getInventory().deleteItem(1046, 43434);
				this.getInventory().deleteItem(1048, 43434);
				this.getInventory().deleteItem(1050, 43434);
				this.getInventory().deleteItem(13740, 43434);
				this.getInventory().deleteItem(13742, 43434);
				this.getInventory().deleteItem(13744, 43434);
				this.getInventory().deleteItem(13746, 43434);
				this.getInventory().deleteItem(13748, 43434);
				this.getInventory().deleteItem(13750, 43434);
				//
				this.getBank().removeItem(995);
				this.getBank().removeItem(1038);
				this.getBank().removeItem(1040);
				this.getBank().removeItem(1042);
				this.getBank().removeItem(1044);
				this.getBank().removeItem(1046);
				this.getBank().removeItem(1048);
				this.getBank().removeItem(1050);
				this.getBank().removeItem(13740);
				this.getBank().removeItem(13742);
				this.getBank().removeItem(13744);
				this.getBank().removeItem(13746);
				this.getBank().removeItem(13748);
				this.getBank().removeItem(13750);
				this.setCoins(5000000);
				ecoreset2 = true;
			}
			...
			public void lastIp() {
				out("You have last logged in from " + this.lastIP + " .");
			}
			
			/**
			 * Donator settings, method.
			 */
			
			public void donatorSettings() {
