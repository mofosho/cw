		...
		if (packetId == PING_PACKET) {
			...
			if (player.getInterfaceManager().containsInterface(1108)) 
				player.getFriendsIgnores().setChatPrefix(value);
					...
					player.setYellName(value);
					player.getDialogueManager().startDialogue("SimpleMessage", "Your yell name has been changed to " + player.getYellName() + " ");
			...
			if ((player.getInterfaceManager().containsInterface(762) && player
				...
				if (player.getCoins() == 0) {
					player.out("You have no coins in your money pouch!");
					return;
				}
					if (amnt > player.getCoins()) {
						 player. getPackets().sendRunScript(5561, 0, "n", player.getCoins());
						   player.getPackets().sendRunScript(5560, player.getCoins() - player.getCoins(), "n");
						player.out(MoneyPouch.getFormattedNumber(player.getCoins()) +" coins have been removed from your money pouch.");
						player.getInventory().addItem(995, player.getCoins());
						player.setCoins(0);
						return;
					}
					if (amnt <= player.getCoins())  {
						player.out(MoneyPouch.getFormattedNumber(amnt) +" coins have been removed from your money pouch.");
						  player. getPackets().sendRunScript(5561, 0, "n", amnt);
						   player.getPackets().sendRunScript(5560, player.getCoins() - amnt, "n");
						player.setCoins(player.getCoins() - amnt);
				...
				int itemId = ((Integer) player.getTemporaryAttributtes()
						.remove("bitem")).intValue();
				player.setBonesToMake(amnt);
				player.setBonesOnAltar(true);
				player.setUsingBones(true);
				BoneHandler.handleItemOnObject(player, new WorldObject(44945,
						10, 1, 3085, 3509, 0), itemId, player.getBonesToMake());
