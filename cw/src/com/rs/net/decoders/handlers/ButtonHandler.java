...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream,
		...
		SoulWarsButtons.handleRewards(player, componentId, packetId);
		...
		if (ExchangeHandler.handleButtons(player, interfaceId, componentId)) {
			...
			if (componentId >= 7 && componentId <= 19)
				...
				if (componentId == 20) {
					...
					if (slot == null)
						return;
					player.getTemporaryAttributtes().put("amount_shop", isBuying ? shop.getMainStock()[slot].getAmount() : player.getInventory().getItems().getItems()[slot].getAmount());
					player.getPackets().sendIComponentText(interfaceId, 67, String.valueOf(amount));
