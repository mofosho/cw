...
import com.rs.content.skills.prayer.BoneHandler;
...
import com.rs.game.player.actions.Farming.Plant;
	...
	private static void handleOption1(final Player player, final WorldObject object) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				player.getDialogueManager().startDialogue(
						"AreaDialogue",
						new Object[] { Integer.valueOf(object.getId()),
					...
					player.getInterfaceManager().sendInterface(397);
	                    ...
	                    player.getActionManager().setSkill(new Farming(Plant.TARRMOIN));
	                } else if (id == 8151) {
	                    player.getActionManager().setSkill(new Farming(Plant.GUAM));
	                } else if (id == 8553) {
	                    player.getActionManager().setSkill(
	                            new Farming(Plant.MARRENTILL));
	                } else if (id == 8552) {
	                    player.getActionManager().setSkill(
	                            new Farming(Plant.HARRALANDER));
	                } else if (id == 8576) {
	                    player.getActionManager().setSkill(
	                            new Farming(Plant.TARRMOIN));
	                } else if (id == 8575) {
	                    player.getActionManager().setSkill(
	                            new Farming(Plant.RANARR));
				...
				//start of varrock dungeon
					...
					if (object.getX() == 2971 && object.getY() == 4382)
	                    ...
	                    if (player.getInventory().containsItem(989, 1)) {
	                        int random[][] = {{8, 1}, {3, 1}, {4, 1},
	                            {8, 1}, {3, 1}, {4, 1}, {8, 1},
	                            {995, 2000000}, {20072, 1}, {4012, 1},
	                            {4834, 1}, {536, 1}, {18830, 1},
	                            {6199, 1}, {1215, 1}, {6585, 1},
	                            {8850, 1}, {2577, 1},
	                            {544, 1}, {15332, 1},
	                            {10724, 1}, {10615, 1}, {10629, 1},
	                            {19281, 1}, {767, 11}, {665, 1},
	                            {11732, 1}, {1149, 1}, {1187, 1},
	                            {995, 1}, {995, 1}, {7356, 1},
	                            {15441, 1}, {806, 1}, {809, 1},
	                            {814, 1}, {820, 1}, {880, 1}, {990, 1},
	                            {220, 1}, {230, 1}, {43, 1}, {42, 1},
	                            {3, 1}, {3, 1}, {4, 1}, {8, 1},
	                            {15441, 1}, {806, 1}, {809, 1},
	                            {3, 1}, {3, 1}, {4, 1}, {8, 1},
	                            {6199, 1}, {880, 1}, {990, 1},
	                            {995, 300000}, {3, 1}, {4, 1}, {8, 1},
	                            {3, 1}, {4, 1}, {8, 1}, {3, 1},     {3, 1}, {3, 1}, {4, 1}, {8, 1},     {3, 1}, {3, 1}, {4, 1}, {8, 1},
				     ...
				     int str = player.getSkills().getLevel(Skills.STRENGTH);
		                int atk = player.getSkills().getLevel(Skills.ATTACK);
		                int sum = atk + str;
					 if (sum < 130) {
		                    player.sendMessage("Your attack and strength level combined must be greater than 130.");
		                   return;
		                    ...
		                    switch (player.getX()) {
		                        case 2877:
		                            ...
		                            player.setNextWorldTile(new WorldTile(2876, 3542, 0));
		                            break;
		                        case 2876:
		                    		player.getInterfaceManager().closeOverlay(true);
		                            player.setNextWorldTile(new WorldTile(2877, 3542, 0));
		                            break;
		                    }
		                }
				...
				} else if (id == 15641 || id == 15644) {
				     if (player.getX() > 2847 || player.getX() == 2847) {
                         player.setNextWorldTile(new WorldTile(player.getX() - 1,
                                 player.getY(), 2));
                         player.setInAnimationRoom(false);
                     } else if (player.getX() < 2847) {
                         if (player.getAnimationGameTokens() < 200) {
                             player.sendMessage("You need at least 200 tokens to enter this room.");
                          
                         }
                         player.setInAnimationRoom(true);
                         player.getAnimationGame().enterRoom(player);
                         player.setNextWorldTile(new WorldTile(player.getX() + 1,
                                 player.getY(), 2));
                     }
			...
			public void run() {
				...
				if (!player.getControlerManager().processObjectClick2(object))
                    ...
                    if (player.getSkills().getLevel(Skills.THIEVING) <= 80) {
                        player.getPackets().sendGameMessage(
                                "You need an thieving level of 80 to steal from this stall",
                                true);
                        return;
                    } else if (player.getInventory().getFreeSlots() <= 0) {
                        player.getPackets().sendGameMessage(
			...
			public void run() {
				player.faceObject(object);
			       if (object.getId() == 15621) {
	                    ...
	                    player.getAnimationGame().createWarrior(itemId, object);
	                    return;
	                }
				if (itemId == 1438 && object.getId() == 2452) {
					...
					slashWeb(player, object);
				} else if(itemId == 14639 && object.getId() == 42010) {
				...
				} else if (object.getId() == 44945) {
					player.setBonesToMake(0);
					player.setUsingBones(false);
					player.setBonesOnAltar(false);
					BoneHandler.handleItemOnObject(player, object, itemId,
							player.getBonesToMake());
					return;
