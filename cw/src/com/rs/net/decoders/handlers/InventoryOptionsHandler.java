...
import com.rs.content.utils.FlowerGame;
import com.rs.content.utils.Lamps;
import com.rs.content.utils.misc.TreasureTrails;
...
import com.rs.game.player.actions.GodSwordCreation;
...
import com.rs.game.player.actions.ShieldCreation;
...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption1(Player player, final int slotId,
		...
		Lamps.handleLamps(player, itemId);
		...
		if (itemId == 2677) {
			TreasureTrails.handleClues(player);
		}
		
		 if (itemId == 4155)
			player.getDialogueManager().startDialogue("EnchantedGemDialogue");
		if (Pots.pot(player, item, slotId))
			return;
//Squeal Of Fortune Tickets
		if (itemId == 24154) {
			player.spins += 1;
		...
		if (itemId == 24155) {
			player.spins += 2;
			player.refreshSqueal();
			player.getPackets().sendGameMessage("You have received double spins on the Squeal of Fortune!");
	...
	public static void handleItemOnItem(final Player player, InputStream stream) {
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE
			...
			Item itemUsed = player.getInventory().getItem(fromSlot);
			 int usedWith1 = player.getInventory().getItem(toSlot).getId();
	            int itemUsed1 = player.getInventory().getItem(fromSlot).getId();
			...
			int herblore = Herblore.isHerbloreSkill(itemUsed, usedWith);
			  GodSwordCreation.handleGodSword(player, itemUsed1, usedWith1);
	            ShieldCreation.handleShieldCreation(player, itemUsed1, usedWith1);
			if (herblore > -1) {
