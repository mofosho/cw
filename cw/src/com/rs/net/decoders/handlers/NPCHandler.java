...
public class NPCHandler {
			...
			public void run() {
				...
				if (npc.getId() == 3709)
					...
					player.getDialogueManager().startDialogue(
 							"LostCityStart",
					new Object[] { Integer.valueOf(npc.getId()),
				...
				player.getDialogueManager().startDialogue("SimpleNPCMessage", 13727,  "The wise old spirit tree is the highest amongs us all.");
				...
				if (npc.getDefinitions().name.contains("clerk")
				 ...
				 ExchangeHandler.mainInterface(player);
				...
				else if (npc.getId() == 2625)
					player.getDialogueManager().startDialogue("Tzhaar",
							new Object[] { Integer.valueOf(npc.getId()) });
					...
					else if (npc.getId() == 13823) {
						player.getDialogueManager().startDialogue("SkillTransportation", npc.getId());
					npc.setNextForceTalk(new ForceTalk("Talk to me to teleport to any of my skilling locations!"));
	...
	public static void handleOption2(final Player player, InputStream stream) {
		...
		if (npc.getDefinitions().name.contains("Banker")
			...
			npc.setNextForceTalk(new ForceTalk("I can bank your items for you!"));
			return;
		}
		player.setCoordsEvent(new CoordsEvent(npc, new Runnable() {
			@Override
